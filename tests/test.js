...
const { app, server, people } = require("../app");
...
describe("People", () => {
    ...
    it(`should return an array of person entries of length ${
      ...
      chai
        ...
        .end((err, res) => {
          ...
          res.body.data.length.should.be.eql(this.index + 1);
          done();
        });
    });
  });
  describe("get /apl/v1/people/:id", () => {
    it("should return the entry corresponding to the last person added.", (done) => {
      chai
        .request(app)
        .get(`/api/v1/people/${this.index}`)
        .end((err, res) => {
          res.should.have.status(200);
          res.body.person.name.should.be.eql("Aiman");
          done();
        });
    });
